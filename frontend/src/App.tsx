...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { currentUser, hasCompletedProfile, isAdmin } = useAuth();
      ...
      <div className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${
        ...
        <div className="p-4">
          ...
          <nav className="space-y-2">
            ...
            {/* Profile Creation Links - Only show if user doesn't have a completed profile */}
...
const AppRoutes: React.FC = () => {
  ...
  // Handle redirects based on user profile status
  React.useEffect(() => {
    if (currentUser) {
      if (userType && hasCompletedProfile) {
        // Existing user with completed profile - redirect to their dashboard
        const currentPath = window.location.pathname;
        if (currentPath === '/' || currentPath === '/create-user') {
          if (userType === 'student') {
      ...
      } else if (!userType && !hasCompletedProfile) {
        // New user without profile - redirect to user type selection
        const currentPath = window.location.pathname;
        if (currentPath === '/') {
  ...
  }, [currentUser, userType, hasCompletedProfile, navigate]);
